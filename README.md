...
Live at `https://haseebasif7.github.io/`, served from the `main` branch via
GitHub Pages.
...
If you later buy a custom domain, add a `CNAME` file containing the bare
domain and update the absolute URLs (see below).
...
- The "Book a call" link points to `https://cal.com/haseebasif`. If the
  Cal.com username ever changes, find and replace it across `index.html`,
  `projects.html`, `research.html`, and `cv.html` (6 occurrences total).
